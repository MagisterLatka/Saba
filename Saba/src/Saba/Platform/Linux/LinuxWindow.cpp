...
void LinuxWindow::Init(const WindowProps& props) {
    ...
    SB_VK_GRAPHICS_CALL_INFO(glfwCreateWindowSurface(context->GetInstance(), m_Window, nullptr, &m_Surface), "Failed to create Vulkan window surface");
    ...
    VkBool32 presentSupported = 0u;
    vkGetPhysicalDeviceSurfaceSupportKHR(context->GetPhysicalDevice(), context->GetGraphicsFamilyIndex(), m_Surface, &presentSupported);
    SB_CORE_ASSERT(presentSupported != 0u, "Device graphics queue family incapable of presenting frames");
...
LinuxWindow::SwapChainSupportDetails LinuxWindow::QuerySwapChainSupport(VkPhysicalDevice device) {
...
void LinuxWindow::CreateSwapChain(SwapChainSupportDetails& swapChainSupport) {
    auto context = VulkanContext::GetContextFromApplication();
    ...
    SB_VK_GRAPHICS_CALL_INFO(vkCreateSwapchainKHR(context->GetLogicalDevice(), &createInfo, nullptr, &m_SwapChain), "Failed to create Vulkan swap chain");
    ...
    for (uint32_t i = 0; i < m_SwapChainImageViews.size(); ++i) {
        ...
        SB_VK_GRAPHICS_CALL_INFO(vkCreateImageView(context->GetLogicalDevice(), &imageViewCreateInfo, nullptr, &m_SwapChainImageViews[i]),
